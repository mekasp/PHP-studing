...
Cookie - это временные файлы для хранения пользовотельской информации

Для установки Cookie сервер должен отправить HTTP заголовок Set-Cookie в HTTP ответе клиенту.
PHP умеет устанавливать заголовок COOKIE в ответ используя функцию setcookie()

Бразуер при обращении к сайту будет добавлять заголовок cookie со всеми куками установленными для сайта.

УСТАНОВКА COOKIE ЧЕРЕЗ PHP:

setcookie('имя куки', 'значение,которое хранится в куки', 'время в формате unix', 'путь куки')

*/

/*

Session - фича от php, основанна на http cookie

Пользовательские данные хранятся на стороне сервера во временных файлах, но работают сессии используя куки.
Устанавливается куки на сторону клиента, которая хранит в себе ID файла сессии, файл с таким ID будет хранится на сервере,
на который клиент обращается и в этом файле будут данные пользователя.
В этом и есть особенность сессий, так как данные хранятся на сервере - они лучше защищены.

УСТАНОВКА СЕССИЙ

session_start(); - эта функция отправит заголовок ответа Set-Cookie , который установит Cookie, которая будет
хранить ID сессии. Функция session_start должна быть определена до какого либо HTML кода или текста.
Сразу после вызова функции session_start можно пользоваться глобальным массивом $_SESSION
