...
Подключение к MySQL серверу:
    -чтобы подключиться MySQL серверу,работающему на текущей локальной машине - mysql - u user -p(после ввести пароль)
    -чтобы подключиться к удаленному серверу нужно добавить ip адрес хоста - mysql -h host -u user -p
    -чтобы отключиться от MySQL сервера - quit

Синтаксис:
    Экранирование пример:
        INSERT INTO books SET `name` = "Книга \"Атлант расправил плечи\" 1 часть";
            \ - выступает в роли комментария,что позволяет создать запись в таблицу,в противном случае "Атлант расправил плечи" будет восприниматся как разрыв значения

PRIMARY KEY - первичный ключ,idшник строки в основной таблице

FOREIGN KEY - первичный ключ второй таблицы для связывания с основной(таблица и которой делается выборка)
...
Запросы к таблицам MySQL:
    INSERT INTO table_name SET column_name = 1; - инсерт одной строки
    INSERT INTO table_name (column_name,column_name) VALUES ('hello',1),('hi',2); - множественный инсерт
    UPDATE table_name SET column_name = 1 - все строчки
    UPDATE table_name SET column_name = 1 WHERE column_name = 2 - конкретной строчки
    ...
    Запросы на выборку данных:
        ...
        Лимит строк:
            ...
            SELECT * FROM table_name LIMIT 10(пропускаются 10 строк сначала),10(и выбираются 10 спрок после пропущенных)
...
Движки таблиц:
    Самые популярные движки в MySQL:
        InnoDB - поддержка транзакций,блокировка на уровне строк,более устойчив к поломка(движок по умолчанию)
        MyISAM - не поддерживает транзакций,блокировка на уровне таблиц,в некоторы случаях работает быстрее
...
Транзакции: - безопасный функционал для проведения ответственных операций,изменения не зальются пока не подтвердят изменение запросов
    TRANSACTION BEGIN;

    ...любое кол-во запросов
